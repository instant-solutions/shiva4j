...
import at.tripwire.shiva4j.objects.Track.MimeType;
...
public class Shiva4J {
	...
	private static final String TRACK_FILES = "files";
	private static final String TRACK_AUDIO_MP3 = "audio/mp3";
	private static final String TRACK_AUDIO_OGG = "audio/ogg";
	...
	/**
	 * Create a new <code>Shiva4J</code> instance using shiva server's host and
	 * port. Additionally add a root uri if your shiva server isn't mounted at
	 * root. The built-up url will look like http://host:port/rootUri
	 * 
	 * @param host
	 *            The shiva server's host.
	 * @param port
	 *            The shiva server's port.
	 * @param rootUri
	 *            The shiva server's rootUri.
	 */
	public Shiva4J(String host, int port, String rootUri) {
		this.baseUri = "http://" + host + ":" + port + "/" + rootUri + "/";
		this.httpClient = new DefaultHttpClient();
	}
	...
	private Track parseTrack(JSONObject obj) throws JSONException {
		...
		// files
		JSONObject filesObj = obj.getJSONObject(TRACK_FILES);
		HashMap<MimeType, String> files = new HashMap<MimeType, String>();
		files.put(MimeType.AUDIO_MP3, filesObj.getString(TRACK_AUDIO_MP3));
		files.put(MimeType.AUDIO_OGG, filesObj.getString(TRACK_AUDIO_OGG));
